class Nivel5 extends Phaser.Scene {
  ...
  create() {
    ...
    const btnBLE = this.add
      .text(btnBLEX, btnBLEY, "Conectar BT", {
        fontFamily: "Planes_ValMore",
        fontSize: tamanhoFonteBT + "px",
        fill: "#ffffff",
  ...
  configurarEscutaEventos() {
    ...
    this.botaoContinuar.on("pointerdown", () => {
      ...
      if (!this.isPaused && this.contadorClique < 7) {
        ...
        // Chama a função pontuacaoMaxima() para mostrar a tela de vitória
  ...
  processarCliqueNaPata() {
    ...
    // Se completou o nível, mostra mensagem final (mas não chama pontuacaoMaxima ainda)
  ...
  atualizarVisibilidadePata() {
    ...
    if (
      this.time.now - this.ultimaTrocaVisibilidade >
  ...
  atualizarPadraoMovimento() {
    ...
    if (
      this.time.now - this.tempoTrocaMovimento >
      this.intervaloTrocaMovimento
    ) {
  ...
  textoTutorial1() {
    this.textoTutorial = this.add.text(
      this.centroX,
      this.altura * 0.1,
      "Nível 5: A dificuldade aumenta, fique atento!",
      {
        fontFamily: "Planes_ValMore",
        fontSize: Math.max(20, Math.floor(35 * this.escalaTexto)) + "px",
        fill: "#ffffff",
  ...
  textoTutorial2() {
    this.textoTutorial = this.add.text(
      this.centroX,
      this.altura * 0.1,
      "Prepare a recompensa e depois toque no botão para continuar.",
      {
        fontFamily: "Planes_ValMore",
        fontSize: Math.max(20, Math.floor(35 * this.escalaTexto)) + "px",
        fill: "#ffffff",
  ...
  textoTutorial3() {
    this.textoTutorial = this.add.text(
      this.centroX,
      this.altura * 0.1,
      "Parabéns! Você completou o nível 5!\nClique no botão para continuar.",
      {
        fontFamily: "Planes_ValMore",
        fontSize: Math.max(20, Math.floor(35 * this.escalaTexto)) + "px",
        fill: "#ffffff",
  ...
  pontuacaoMaxima() {
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        localStorage.setItem("nivel5Completo", true); // Marca o nível como completo no localStorage
        ...
        btnProximoNivel.on("pointerdown", () => {
          this.scene.start("Menu"); // Mude para o nome da próxima cena
        });
